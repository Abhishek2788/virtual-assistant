...
const SignIn = () => {
  ...
  return (
    <div className='w-full h-screen bg-cover flex justify-center items-center' style={{backgroundImage: `url(${robo})`}}>
      <form className='w-[90%] h-[700px] max-w-[500px] bg-[#05050520] backdrop-blur shadow-black flex flex-col items-center justify-center gap-5 px-5' onSubmit={handleSignIn}>
        ...
        <input type="text" placeholder='Email' className='w-full h-[60px] outline-none border-2 border-white bg-transparent text-white placeholder-gray-300 px-5 py-2.5 rounded-full' required onChange={(e)=>setEmail(e.target.value)} value={email} />
         ...
         <button className='min-w-[150px] h-[60px] mt-7 text-black font-semibold bg-white rounded-full text-[19px]' disabled={loading}>{loading?"Loading....":"Sign in"}</button>
         <p className='text-white text-[18px]'>Wan't to SignUp ? <span className='text-blue-400 cursor-pointer' onClick={()=>navigate("/signup")}>Sign Up</span></p>
      </form>
    </div>
  )
}
